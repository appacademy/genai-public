...
class OllamaClient:
    ...
    def generate_stream(
        ...
        """
        if not self.check_health():
            yield {"error": "Ollama API not available", "done": True}
            return
    ...
    def chat_completion_stream(
        ...
        """
        Stream a ChatCompletion-style request (like OpenAI) for Ollama.
        ...
        Yields:
            Dict: Each chunk of the response, formatted similar to OpenAI's streaming format.
        """
        # Convert the messages format to a single prompt for Ollama
        prompt = self._format_chat_messages(messages)

        # Stream response
        for token in self.generate_stream(prompt, temperature, max_tokens):
            if isinstance(token, dict) and "error" in token:
                yield token
                return

            # Format each token to match OpenAI's streaming format
            yield {
                "choices": [
                    {
                        "delta": {"content": token},
